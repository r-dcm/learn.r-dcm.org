  ...
  \(x, items, s_wt = 0.3, total, single) {
    bind_rows(
      ...
      items |>
        ...
        slice_sample(
          n = (items_per_att - single_att_items),
          weight_by = weight,
          replace = TRUE
        ) |>
        select(-weight)
...
intercepts <- all_params |>
  ...
  mutate(prob = runif(n = n(), min = 0.05, max = 0.45), value = logit(prob))
...
maineffects <- all_params |>
  ...
  mutate(
    prob = map2_dbl(num_att, int_prob, \(att, prob) {
      cur_min <- ifelse(att == 1, max(prob, 0.60), max(prob, 0.2))
      cur_max <- ifelse(att == 1, 0.95, .7)
...
interactions <- all_params |>
  ...
  mutate(
    ints = map(params, \(x) {
      int_prob = runif(n = 1, min = max(max(x$mef_prob, 0.6)), max = 0.95)
      tibble(
        prob = int_prob,
        value = logit(int_prob) - unique(x$int_value) - sum(x$mef_value)
      )
    })
  ) |>
  select(-params) |>
...
example_dat <- expand_grid(
  resp_id = seq_len(nrow(profiles)),
  ...
  summarize(
    log_odds = sum(final_value, na.rm = TRUE),
    .by = c(resp_id, item_id)
  ) |>
  mutate(
    prob = inv_logit(log_odds),
    rand = runif(n = n()),
    score = case_when(rand < prob ~ 1L, rand >= prob ~ 0L)
  ) |>
...
initial_irt <- mirt(
  data = select(example_dat, -resp_id),
...
profiles <- dcm_profile |>
  ...
  mutate(
    min_theta = map_dbl(resp, \(x) min(x$theta)),
...
resp_albums <- tribble(
  ...
  mutate(across(-c(album, era), \(x) {
    case_when(x == "xmark" ~ 0L, x == "check" ~ 1L)
  })) |>
  mutate(
    target_theta = case_when(
      era == "Fearless" ~ -2.500,
      era == "Debut" ~ -2.080,
      era == "Red" ~ -1.670,
      era == "Speak Now" ~ -1.400,
  ...
  left_join(
    profiles,
    join_by(songwriting == att1, production == att2, vocals == att3)
  ) |>
  mutate(
    selection = map2(resp, target_theta, \(r, t) {
      r |>
        slice_min(n = 1, order_by = abs(theta - t), with_ties = FALSE)
    })
  ) |>
  unnest(selection) |>
  select(
...
albums <- map_dfr(playlists, \(x) {
  tracks <- get_playlist(playlist_id = x)

  tibble(
    album = tracks$tracks$items$track.album.name,
  ...
  summarize(artist = paste(unique(artist), collapse = ","), .by = album) |>
...
taylor_data <- example_dat |>
  mutate(
    album = sample(albums$album, size = n(), replace = FALSE),
    .after = resp_id
  ) |>
  left_join(select(resp_albums, resp_id, ts_alb = album), join_by(resp_id)) |>
  mutate(album = case_when(is.na(ts_alb) ~ album, !is.na(ts_alb) ~ ts_alb)) |>
  select(-resp_id, -ts_alb) |>
  mutate(
    album = factor(album, levels = c(resp_albums$album, sample(albums$album)))
  ) |>
  arrange(album) |>
...
popularity <- tribble(
  ...
  mutate(
    popularity = map_int(album_uri, function(.x) {
      album <- get_album(.x)
      album$popularity
    })
  )
...
album_meta <- here("figure", "taylor", "eras") |>
  ...
  rename(img = value) |>
  mutate(
    img = str_replace_all(img, "^.*(?=figure)", ""),
    img = path_file(img),
    era = path_ext_remove(img),
    era = str_replace(era, "[0-9]{2}-", ""),
    era = str_replace_all(era, "-", " "),
    era = case_when(
      era %in% c("reputation", "folklore", "evermore") ~
        era,
      era == "ttpd" ~ "TTPD",
      str_detect(era, "tv") ~ str_replace(str_to_title(era), "Tv", "(TV)"),
  ...
  left_join(select(popularity, -album_uri), join_by(album == album_name)) |>
  left_join(select(taylor_albums, -ep), join_by(album == album_name)) |>
...
predict(taylor_dcm, summary = FALSE) |>
  ...
  mutate(across(where(is.double), \(x) {
    case_when(x < 0.5 ~ 0L, x >= 0.5 ~ 1L)
  })) |>
  left_join(
    select(resp_albums, album, era, songwriting:vocals, theta),
